...
namespace S3.Common.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, S3Exception, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is S3Exception s3Exception && onError != null)
                        {
                            var rejectedEvent = onError(message, s3Exception);
